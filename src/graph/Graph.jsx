...
export default function Graph({ collections }) {
  Chart.register(ArcElement, CategoryScale);

  const typeTotals = collections.reduce((totals, collection) => {
    collection.pokemons.forEach((pokemon) => {
      pokemon.types.forEach((type) => {
        if (totals[type.type.name]) {
          totals[type.type.name]++;
        } else {
          totals[type.type.name] = 1;
        }
      });
    });
    return totals;
  ...
  const backgroundColors = Object.keys(typeTotals).map((type) => {
    return colors[type].bg;
  });
